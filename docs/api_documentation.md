...
Lucky Event 화면의 테마(라이트 / 다크)를 설정합니다. (`1.1.9` 이상)

**적용 범위는 네이티브 영역입니다.** 화면 배경, 상태바 아이콘 대비, SDK 가 표시하는 안내 팝업이
대상이며, 이벤트 웹 페이지 자체는 웹에서 정한 대로 표시됩니다.

`showLuckyEvent`, `showLuckyEventUrl` 보다 먼저 호출해야 하며, 이미 떠 있는 화면에는 반영되지 않습니다.
...
> **`1.1.9` 부터 기본값이 `UIUserInterfaceStyleUnspecified` 입니다.**
